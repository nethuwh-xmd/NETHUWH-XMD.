const express = require('express');
    ...
    ╰───────────────❏
       ...
       *🖥️ Channel React 🄳🄾🄽🄴 ✅*
        ...
        await socket.sendMessage(sender, {
            image: { url: "https://files.catbox.moe/xw7gz1.jpg" },
            caption: text,
            footer: "🄽🄴🅃🄷🅄🅆🄷 🅇🄼🄳 💥",
            buttons: buttons,
            headerType: 4
    ...
    //=======================================
                case 'ping': {     
                    ...
                    let ping = await socket.sendMessage(sender, { text: '*_Pinging to 𝗡𝗘𝗧𝗛𝗨𝗪𝗛 𝗫𝗠𝗗 🪢 Module..._* ❗' });
    ...
    *ᴺᴱᵀᴴᵁᵂᴴ ˣᴹᴰ ᴹᴵᴺᴵ ᴮᴼᵀ ⱽ1 ᴶᴵᴰ ˢᵁᶜᶜᴱˢˢ 🌝:*
            `.trim()
        });
        break;
    }
...
case 'ig': {
    ...
    try {
        ...
        // Free API example
        const apiUrl = `https://vihangayt.me/download/ig?url=${encodeURIComponent(link)}`;
        ...
        // Send each media item with buttons
...
*${footer}*`;
    ...
    const menuMsg = await conn.sendMessage(m.chat, {
        image: { url: logo },
        caption: desc
    }, { quoted: m });
    ...
    conn.ev.on('messages.upsert', async (msgUpdate) => {
        ...
        let opt = msg.message.extendedTextMessage.text.trim();
        let cmdMap = {
            '1': 'derana',
            '2': 'hiru',
            '3': 'bbc',
        ...
        if (cmdMap[opt]) {
            conn.sendMessage(m.chat, { text: `.${cmdMap[opt]}` }, { quoted: m });
        }
    });
}
break;
...
*${footer}*`;
        ...
        if (data.result.image) {
            await conn.sendMessage(m.chat, { image: { url: data.result.image }, caption: cap }, { quoted: m });
        } else {
            reply(cap);
        }
    } catch {
        reply(`❌ Error fetching Derana news`);
    }
}
break;
...
case 'hiru': {
    try {
        let res = await axios.get(api.HIRU_API);
        let data = res.data;
        if (!data.title) return reply('*No News Available* ❗');
...
*${footer}*`;
        ...
        if (data.image) {
            await conn.sendMessage(m.chat, { image: { url: data.image }, caption: cap }, { quoted: m });
        } else {
            reply(cap);
        }
    } catch {
        reply('*Error fetching Hiru news ❗*');
    }
}
break;
...
*${footer}*`;
        ...
        reply(`❌ Error fetching LankaDeepa news`);
    }
}
break;
...
*${footer}*`;
        ...
        reply(`❌ Error fetching Siyatha news`);
    }
}
break;
...
*${footer}*`;
        ...
        reply(`❌ Error fetching Neth news`);
    }
}
break;
...
*${footer}*`;
        ...
        reply(`❌ Error fetching Lanka news`);
    }
}
break;
...
*${footer}*`;
        ...
        reply(`❌ Error fetching Gossip Lanka news`);
    }
}
break;
...
case 'worldnews': {
    try {
        let apiKey = "0f2c43ab11324578a7b1709651736382";
        ...
        for (let i = 0; i < Math.min(articles.length, 5); i++) {
            let art = articles[i];
            let cap = `📰 *${art.title || "No Title"}*
⚠️ _${art.description || "No Description"}_
🔗 _${art.url || "No URL"}_

*${footer}*`;

            if (art.urlToImage) {
                await conn.sendMessage(m.chat, { image: { url: art.urlToImage }, caption: cap }, { quoted: m });
            } else {
                reply(cap);
            }
        }
    } catch {
        reply("Could not fetch world news.");
    }
}
break;
                    }
